...
const Template: Story<LayerProps> = (args: LayerProps) => {
  ...
  const [lossVisible, setLossVisible] = useState(true);
  const minZoom = 0;
  const maxZoom = 20;
  const [viewport, setViewport] = useState({});
  const [bounds] = useState({
    bbox: [48.831181, -13.983606, 48.97221, -13.856368],
    options: {
      duration: 0,
    }
  });
      ...
      new MapboxLayer(
        {
          ...
          getTileData: (tile) => {
            ...
            const url = `https://storage.googleapis.com/geo-ai/Redes/Tiles/Tsaratanana/APNGs/Prediction/${z}/${x}/${y}.png`;
      ...
      new MapboxLayer(
        {
          ...
          data: 'https://tiles.globalforestwatch.org/umd_tree_cover_loss/v1.9/tcd_30/{z}/{x}/{y}.png',
          tileSize: 256,
          visible: true,
          opacity: lossVisible ? 0.35 : 0,
          refinementStrategy: 'no-overlap',
          decodeParams: {
            startYear: 2001,
            endYear: 2001 + frame,
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <label htmlFor='#loss-layer'>Loss Layer</label>
          <input
            type="checkbox"
            checked={lossVisible}
            onChange={(e) => {
              setLossVisible(e.target.checked)
      ...
      <Map
        ...
        initialViewState={{
          bounds: [48.831181, -13.983606, 48.97221, -13.856368],
        }}
